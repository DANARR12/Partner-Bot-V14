...
> 📮 **Post This Link in Your Server To** ${link}`}).catch(async (err) => {
            await message.channel.send({content : `> **${message.author} Your Server Posted in ${share}**`});
        })
    }).catch(async (err) => {
      console.log(err)
...
// Voice channel functionality disabled - requires additional system dependencies
